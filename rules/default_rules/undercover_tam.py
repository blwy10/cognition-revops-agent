...
def undercover_tam_metric(acct: Account, opportunities: list[Opportunity]) -> dict:
    total_opps = len([opp for opp in opportunities if opp.accountId == acct.id])
    if total_opps == 0:
        return {'pipeline': 0, 'tam': 0, 'coverage': 100}
    total_opp_amt = sum(opp.amount for opp in opportunities if opp.accountId == acct.id)
...
def undercover_tam_responsible(acct: Account) -> str:
    return acct.owner
